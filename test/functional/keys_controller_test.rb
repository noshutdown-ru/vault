...
class KeysControllerTest < Vault::ControllerTest
  ...
  self.file_fixture_path = Rails.root.join('plugins', 'vault', 'test', 'fixtures')
  ...
  def test_index_search
    ...
    get :index, params: { project_id: 1, search: '1' }
  ...
  def test_unpriv_create
    @request.session[:user_id] = 3
    post :create, params: { project_id: 1, key: { name: 'root', body: '123456' } }
  ...
  def test_delete
    ...
    delete :destroy, params: { project_id: 1, id: 3 }
    ...
    assert_nil key
    refute File.exist?("#{Vault::KEYFILES_DIR}/server.key")
  end
  ...
  def test_crossproject_edit
    @request.session[:user_id] = 2
    get :edit, params: { project_id: 1, id: 2 }
  ...
  def test_update_with_new_file
    ...
    put :update, params: { id: 3, project_id: 1, vault_key: { file: fixture_file_upload('keyfile.txt') } }
  ...
  def test_update
    @request.session[:user_id] = 2
    put :update, params: { id: 1, project_id: 1, vault_key: { name: 'database', login: 'me', type: 'Vault::KeyFile' } }
  ...
  def test_unpriv_update
    @request.session[:user_id] = 3
    put :update, params: { id: 1, project_id: 1, vault_key: { name: 'database', body: '123456', login: 'me' } }
    assert_response 403
  end

  def test_crossproject_update
    @request.session[:user_id] = 2
    put :update, params: { id: 2, project_id: 1, vault_key: { name: 'database', body: '123456', login: 'me' } }
    assert_response :redirect
  end
  ...
  def test_crossproject_show
    @request.session[:user_id] = 2
    get :show, params: { project_id: 1, id: 2 }
    assert_response :redirect
  end
  ...
  def test_create
    @request.session[:user_id] = 2
    post :create, params: { project_id: 1, vault_key: { name: 'database', type: 'Vault::Password', login: 'me', body: 123456 } }
  ...
  def test_create_with_tags
    @request.session[:user_id] = 2
    post :create, params: { project_id: 1, vault_key: { name: 'router', type: 'Vault::Password', tags: "ssh, cisco" } }
  ...
  def test_update_tags
    @request.session[:user_id] = 2
    put :update, params: { id: 1, project_id: 1, vault_key: { tags: "mysql" } }
  ...
  def test_tag_search
    @request.session[:user_id] = 2
    get :index, params: { project_id: 1, search: "#ftp" }
  ...
  def test_upload_keyfile
    ...
    post :create, params: { project_id: 1, vault_key: { name: 'database', type: 'Vault::KeyFile', file: fixture_file_upload('keyfile.txt') } }
    ...
    key.decrypt!
    assert_equal 'Vault::KeyFile', key.type
    assert File.exist?("#{Vault::KEYFILES_DIR}/#{key.file}"), "File: #{key.file} should be at Rails root keyfiles dir"
  end
